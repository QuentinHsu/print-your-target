...
function analyzeJavaScriptSelection(
  ...
  return { logMessage, logMethod, variableName };
}

// 辅助函数：生成 console 语句
function generateConsoleStatement(logMethod: string, logMessage: string, variableName: string, selectedText: string): string {
  const trimmedText = selectedText.trim();
...
function findJavaScriptInsertionPoint(
  ...
  return { finalStatement, insertPosition };
}
...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand('print-your-target.addLogStatement', () => {
      ...
      case 'javascript':
        {
          ...
          const { insertPosition: pos, finalStatement } = findJavaScriptInsertionPoint(document, editor, selection, baseLog);
  ...
  const deleteLogDisposable = vscode.commands.registerCommand('print-your-target.deleteLogStatements', () => {
    ...
    // 根据语言类型构建正则表达式
    const logRegex: RegExp | null = null;
    ...
    match = lineRegex.exec(documentText);
    while (match !== null) {
      ...
      const rangeToDelete = line.lineNumber === document.lineCount - 1 ? line.range : line.rangeIncludingLineBreak;
